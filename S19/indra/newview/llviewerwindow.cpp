...
void LLViewerWindow::hoverPickCallback(const LLPickInfo& pick_info)
{
	...
	LLViewerObject* obj = gViewerWindow->mHoverPick.getObject();
	LLViewerObject* parent = (obj && obj->getVolume()) ? obj->getRootEdit() : NULL;

	if ((obj && obj->getVolume() && obj->flagHandleTouch()) ||
		(parent && parent->getVolume() && parent->flagHandleTouch()))
	{
		gPipeline.setHighlightObject(parent->mDrawable);
	}
	else
	{
