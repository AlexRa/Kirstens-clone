...
{
#define TEST_LLCC(T, V) if((T)mySetting_##T != V) llwarns << "Fail "#T << llendl
	TEST_LLCC(U32, 666);
	TEST_LLCC(S32, (S32)-666);
	TEST_LLCC(F32, (F32)-666.666);
	TEST_LLCC(bool, true);
	TEST_LLCC(BOOL, FALSE);
	if((std::string)mySetting_string != "Default String Value") llwarns << "Fail string" << llendl;
...
//There's no LLSD comparsion for LLCC yet. TEST_LLCC(LLSD, test_llsd); 
	...
	if((std::string)test_BrowserHomePage != "http://www.secondlife.com") llwarns << "Fail BrowserHomePage" << llendl;
}
