...
bool FlashControl::load(const Ogre::String& movieFilename)
{
	std::string moviePath = HikariManager::Get().basePath + movieFilename;
   return loadDirect(moviePath);
}

bool FlashControl::loadDirect(const Ogre::String& movieFilename)
{
   if (flashInterface->FlashVersion() < 0x90000)
      return false;

   std::string moviePath = movieFilename;
