...
S32 LLQueuedThread::updateQueue(U32 max_time_ms)
{
	...
	else
	{
		while (pending > 0)
		{
			...
			// reX: It probably doesn't do good to loop requests more than once, so cap iterations in addition to time
			iterations++;
			if (max_time && (timer.getElapsedTimeF64() > max_time || iterations >= pending))
				break;
		}
	}
	return pending;
...
// 				check();
		...
		else
		{
			...
			{
				// reX: Sleep only if threaded, to prevent mysterious framerate loss
				if (mThreaded) 
