...
// reX: new function
bool LLBase64::decode(const std::string &input, unsigned char *buffer, size_t buffer_size)
{
   int len = apr_base64_decode_len(input.c_str());
   if (len != buffer_size || buffer_size <= 0 || buffer == 0 || input.empty())
      return false;

   apr_base64_decode_binary(buffer, input.c_str());

   return true;
}

// reX: new function
int LLBase64::decode_len(const std::string &input)
{
   return apr_base64_decode_len(input.c_str());
}
