...
LLDir_Mac::LLDir_Mac()
{
	...
	OSStatus		error = noErr;
	FSRef			fileRef;
	CFStringRef		secondLifeString = CFSTR("SecondLife"); // Kirsten leave this for the time for mac builds!
	...
	if (executableURLRef != NULL)
	{
		...
		if (error == noErr)
		{
			...
			(void)CFCreateDirectory(&cacheDirRef, CFSTR("OpenLife"),NULL); //Kirsten Create Correct Cache file directory on MAc systems?
		}
