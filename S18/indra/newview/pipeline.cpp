...
void LLPipeline::bindDeferredShader(LLGLSLShader& shader, U32 light_index, LLRenderTarget* gi_source, LLRenderTarget* last_gi_post)
{
...
void LLPipeline::renderDeferredLighting()
{
	...
	U32 cur_gi_map = gFrameCount%2;

	bindDeferredShader(gDeferredPostGIProgram, 0, &mGIMap, &mGIMapPost[(cur_gi_map+1)%2]);
	{
		LLGLDisable blend(GL_BLEND);
		LLGLDepthTest depth(GL_FALSE);
		mGIMapPost[cur_gi_map].bindTarget();
		stop_glerror();
		glDrawArrays(GL_TRIANGLE_STRIP, 0, 3);
		stop_glerror();
		mGIMapPost[cur_gi_map].flush();
	}
	...
	{
		bindDeferredShader(gDeferredSoftenProgram, 0, &mGIMapPost[cur_gi_map]);	
		{
			LLGLDepthTest depth(GL_FALSE);
			LLGLDisable blend(GL_BLEND);
			LLGLDisable test(GL_ALPHA_TEST);
...
void LLPipeline::generateGI(LLCamera& camera, LLVector3& lightDir, std::vector<LLVector3>& vpc)
{
	...
	mGIRange.mV[0] = lrad/(max.mV[0]-min.mV[0]);
	mGIRange.mV[1] = lrad/(max.mV[1]-min.mV[1]);
	mGILightRadius = lrad;
