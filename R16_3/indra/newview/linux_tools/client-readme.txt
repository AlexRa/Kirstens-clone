Open Life - Linux Beta README
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

This document contains information about the Open Life Linux
client, and isn't meant to serve as an introduction to Open
Life itself - please see <http://www.Openlife.com/whatis/>.
...
Hi!  This is a BETA release of the Open Life client for Linux.
...
work quite well 'out of the box' for accessing Open Life.
...
to the main Open Life world and changes you make there are permanent.

You will have either obtained this client from Openlife.com (the official
site) or from a third-party packager; if you encounter any problems then
we recommend trying the latest official builds from Openlife.com which are
updated often.
...
Minimum requirements:
    ...
    * Video/Graphics Card:
      ...
      **NOTE**: Open Life absolutely requires you to have recent, correctly-
      ...
      TROUBLESHOOTING section if you encounter problems starting Open Life.

For a more comfortable experience, the RECOMMENDED hardware for the Open
Life Linux client is very similar to that for Windows, as detailed at:
<https://Openlife.com/corporate/sysreqs.php>


3. INSTALLING & RUNNING
-=-=-=-=-=-=-=-=-=-=-=-

The Open Life Linux client entirely runs out of the directory you have
unpacked it into - no installation step is required.

Run ./Openlife from the installation directory to start Open Life.
...
User data is stored in the hidden directory ~/.Openlife by default; you may
override this location with the OpenLIFE_USER_DIR environment variable if
you wish.
...
* UPDATING - when the client detects that a new version of Open Life
  ...
  new version from the Open Life web site, <http://www.Openlife.com/>.
...
run from.  Most of this is also replicated in ~/.Openlife/logs/OpenLife.log
...
PROBLEM 1:- Open Life fails to start up, with a warning on the console like:
...
PROBLEM 2:- My whole system seems to hang when running Open Life.
...
SOLUTION:- Some residents with ATI cards have reported that running
   'sudo aticonfig --locked-userpages=off' before running Open Life solves
   their stability issues.
SOLUTION:- As a last resort, you can disable most of Open Life's advanced
   graphics features by editing the 'Openlife' script and removing the '#'
   from the line which reads '#export LL_GL_NOEXT=x'

PROBLEM 3:- After I minimize the Open Life window, it's just blank when
   it comes back.
SOLUTION:- Some Linux desktop 'Visual Effects' features are incompatible
   with Open Life.  One reported solution is to use your desktop
...
SOLUTION:- The most common solution is to ensure that you have the 'esd'
   ...
   start Open Life.  Users of Ubuntu (and some other) Linux systems can
...
SOLUTION:- Some window managers eat the Alt key for their own purposes; you
   ...
   properly with mouse actions in Open Life and other applications.
...
The 'Openlife' script which launches Open Life contains some
configuration options for advanced troubleshooters.

* AUDIO - Edit the 'Openlife' script and you will see three audio
  options: LL_BAD_ESD, LL_BAD_OSS, LL_BAD_ALSA.  Open Life tries to
...
We're pleased to have released the Open Life client's source code under
an Open Source license compatible with the 'GPL'.


8. GETTING MORE HELP AND REPORTING PROBLEMS
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

For general help and support with Open Life:
<http://Openlife.com/>
