...
bool idle_startup()
{
	...
	if ( STATE_FIRST == LLStartUp::getStartupState() )
	{
		...
		if ((!gLoginHandler.mFirstName.empty() &&
			...
			|| gLoginHandler.parseDirectLogin(LLStartUp::sURL3DXCommand) )
		{
	...
	{
		//reset the values that could have come in from a url3DX
...
//#define LL_MINIMIAL_REQUESTED_OPTIONS
	...
	if (STATE_LOGIN_AUTHENTICATE == LLStartUp::getStartupState())
	{
		...
		LLUserAuth::getInstance()->authenticate(
			...
			gLastExecEvent,
			requested_options );  //No taking peoples mac address or hardware serial numbers! Kirsten
...
#ifdef LL_WINDOWS
		// rex skype plugin: send own skype address to rex server
		RexSkypePlugin* skype = RexSkypePlugin::GetInstance(); // rex
		if (skype)
         skype->Initialize();
#endif
	...
	if (STATE_INVENTORY_SEND == LLStartUp::getStartupState())
	{
		...
		// Get C- and ownership credit information
	...
	if (STATE_MISC == LLStartUp::getStartupState())
	{
		...
		// if needed, show the C- history window
...
void login_show()
{
	...
	for(int grid_index = GRID_INFO_OPENLIFE; grid_index < GRID_INFO_OTHER; ++grid_index)
	{
...
void update_dialog_callback(S32 option, void *userdata)
{
	...
	// if a sim name was passed in via command line parameter (typically through a URL3DX)
...
std::string LLStartUp::sURL3DXCommand;
...
bool LLStartUp::dispatchURL()
{
	...
	if (!sURL3DXCommand.empty())
	{
		const bool from_external_browser = true;
		LLURLDispatcher::dispatch(sURL3DXCommand, from_external_browser);
	}
