...
//#include "llfloateractionicon.h"
//#include "llfloatereditcameraview.h"
//#include "llfloaterobjectlist.h"
//#include "llfloaterogrepostprocess.h"
//#include "llogrepostprocess.h"
//#include "llfloaterrexikeditor.h"
#include "llfloaterassetbrowser.h"
//#include "AvatarGeneratorScene.h"
#include "rexskypeplugin.h"
//#include "llfloaterlightmap.h"
...
// reX
#ifdef LL_WINDOWS // Kirsten this stops linux and mac builds failing because of skype plugin!

   RexSkypePlugin *skype = RexSkypePlugin::GetInstance();
   if (skype)
   {
      if (skype->skypeInstalled() == false)
      {
         LLMenuItemCallGL* skype = gMenuHolder->getChild<LLMenuItemCallGL>("skype");
         if (skype)
            skype->setEnabled(FALSE);
         else
            llinfos << "Skype menuitem not found!" << llendl;
      }
   }
#endif 
...
void init_debug_ui_menu(LLMenuGL* menu)
{
	menu->append(new LLMenuItemCallGL("URL3DX Test", &handle_url3DX_test));
	...
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		if (new_value)
		{
			...
			if (avatar)
			{
				...
				BOOL is_linden = lastname && !LLStringUtil::compareStrings(lastname->getString(), "Openlife");
...
/*// reX: new function
#if LL_WINDOWS
// Start skype call if skype address known
class LLMakeSkypeCall : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		if(dest && dest->isAvatar())
		{
			std::string skypeAddress = avatar->GetSkypeAddress();
			RexSkypePlugin* skype = RexSkypePlugin::GetInstance();
			if (skype && skypeAddress.length() > 0)
			{
				bool ok  = skype->StartSkypeCall(skypeAddress);
				if (!ok)
				{
					// TODO: error message
				}
...
class LLFloaterVisible : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
        ...
        {
          new_value = gSavedPerAccountSettings.getBOOL("SkypeExposeAddress");
        }
...
#ifdef LL_WINDOWS
   RexSkypePlugin *skype = RexSkypePlugin::GetInstance();
   if (gSavedPerAccountSettings.getBOOL("SkypeExposeAddress") == FALSE)
   {
      gSavedPerAccountSettings.setBOOL("SkypeExposeAddress", TRUE);
      skype->Initialize();
   } else
   {
      gSavedPerAccountSettings.setBOOL("SkypeExposeAddress", FALSE);
      RexSkypePlugin::RemoveInstance(true);
      RexSkypePlugin::CreateInstance();
      skype = 0;
   }
#endif
}
...
void handle_url3DX_test(void*)
{
	...
	const bool open_app_url3DXs = true;
	LLFloaterHtml::getInstance()->show(
		"http://openlife.com/app/search/url3DXs.html",
		"URL3DX Test", 
...
void initialize_menus()
{
	...
	// reX
#if LL_WINDOWS
	// addMenu(new LLMakeSkypeCall(), "Avatar.MakeSkypeCall");
#endif
