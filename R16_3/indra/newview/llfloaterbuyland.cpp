...
void LLFloaterBuyLandUI::updateNames()
{
	...
	if (mIsClaim)
	{
		mParcelSellerName = "OpenLife"; //kirsten
	}
										  ...
										  LLXMLRPCValue params)
{
	...
	{
		transaction_uri = LLViewerLogin::getInstance()->getHelperURI() + "http://util.logingrid.net/l.aspx"; //Build 160 KL
	}
...
void LLFloaterBuyLandUI::refreshUI()
{
	...
	if (!showingError)
	{
		...
		if (haveEnough)
		{
			...
			childSetText("currency_reason", getString("have_enough_credits", string_args));
		}
		else
		{
			LLStringUtil::format_map_t string_args;
			...
			childSetText("currency_reason", getString("not_enough_credits", string_args));
