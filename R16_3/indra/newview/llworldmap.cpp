...
LLWorldMap::LLWorldMap() :
	...
	mURL3DXRegionName(),
	mURL3DXRegionHandle(0),
	mURL3DX(),
	mURL3DXCallback(0),
	mURL3DXTeleport(false)
{
...
void LLWorldMap::sendMapLayerRequest()
{
	...
	{ */   //Kirsten Disabling maplayer by capability get via message system!
...
{
	mURL3DXRegionName = region_name;
	mURL3DXRegionHandle = 0;
	mURL3DX = callback_url;
	mURL3DXCallback = callback;
	mURL3DXTeleport = teleport;
...
void LLWorldMap::sendHandleRegionRequest(U64 region_handle, 
		...
		bool teleport)	// immediately teleport when result returned
{
	mURL3DXRegionName.clear();
	mURL3DXRegionHandle = region_handle;
	mURL3DX = callback_url;
	mURL3DXCallback = callback;
	mURL3DXTeleport = teleport;
...
// 			llinfos << "Map sim " << name << " image layer " << agent_flags << " ID " << image_id.getString() << llendl;
		...
		if(LLWorldMap::getInstance()->mURL3DXCallback != NULL)
		{
			// Server returns definitive capitalization, URL3DX might not have that.
			if ((LLStringUtil::compareInsensitive(LLWorldMap::getInstance()->mURL3DXRegionName, name)==0)
				|| (LLWorldMap::getInstance()->mURL3DXRegionHandle == handle))
			{
				url_callback_t callback = LLWorldMap::getInstance()->mURL3DXCallback;

				LLWorldMap::getInstance()->mURL3DXCallback = NULL;
				LLWorldMap::getInstance()->mURL3DXRegionName.clear();
				LLWorldMap::getInstance()->mURL3DXRegionHandle = 0;

				callback(handle, LLWorldMap::getInstance()->mURL3DX, image_id, LLWorldMap::getInstance()->mURL3DXTeleport);
			}
		}
	}
...
void LLWorldMap::processMapItemReply(LLMessageSystem* msg, void**)
{
	...
	for (S32 block=0; block<num_blocks; ++block)
	{
		...
		switch (type)
		{
			...
			{
				new_item.mToolTip = llformat("%d sq. m. C-%d", new_item.mExtra, new_item.mExtra2);
