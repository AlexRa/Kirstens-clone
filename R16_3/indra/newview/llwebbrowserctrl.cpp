...
////////////////////////////////////////////////////////////////////////////////
void LLWebBrowserCtrl::setOpenAppURL3DXs( bool valIn )
{
	mOpenAppURL3DXs = valIn;
};
...
{
	// don't browse to anything that starts with openlife:// or ol://
	const std::string protocol1 = "openlife://";
	const std::string protocol2 = "ol://";
	...
	if( mOpenLinksInInternalBrowser )
	{
		if ( eventIn.getStringValue().length() )
		{
				 ...
				 LLStringUtil::compareInsensitive( eventIn.getStringValue().substr( 0, protocol2.length() ), protocol2 ) == 0 )
			{
				...
				// not to open openlife:///app/ links. 
...
void LLWebBrowserCtrl::onClickLinkNoFollow( const EventType& eventIn )
{
	...
	if (LLURLDispatcher::isURL3DXCommand(url)
		&& !mOpenAppURL3DXs)
	{
		// block handling of this openlife:///app/ URL
