...
BOOL LLToolPie::pickAndShowMenu(BOOL always_show)
{
	...
	else if (object)
	{
			...
			|| (object->isAttachment() && !object->isHUDAttachment() && !object->permYouOwner()))
		{
            ...
            // reX: skype stuff
			// Skype: hide menu item if there is no skype on this client
			//        disable menu item if targets skype address is unknown
			bool disableSkypeCommand = false;
			bool hideSkypeCommand = false;
			//if (avatar->GetSkypeAddress().length() == 0)
				//disableSkypeCommand = true;
#if LL_WINDOWS
			RexSkypePlugin* skype = RexSkypePlugin::GetInstance();
			if (!skype || !skype->InitOk() )
				hideSkypeCommand = true;
#endif
			if (hideSkypeCommand)
			{
				gPieAvatar->setItemEnabled("Skype call", false);
				gPieAvatar->setItemVisible("Skype call", false);
			}
			else
			{
				gPieAvatar->setItemVisible("Skype call", true);
				if (disableSkypeCommand)
					gPieAvatar->setItemEnabled("Skype call", false);
				else
					gPieAvatar->setItemEnabled("Skype call", true);
			}
