...
const F32 TELEPORT_RENDER_DELAY = 1.f; // Max time a teleport is allowed to take before we raise the curtain
const F32 TELEPORT_ARRIVAL_DELAY = 1.f; // Time to preload the world before raising the curtain after we've actually already arrived.
BOOL		 gTeleportDisplay = FALSE;
LLFrameTimer gTeleportDisplayTimer;
LLFrameTimer gTeleportArrivalTimer;
const F32		RESTORE_GL_TIME = 1.f;	// Wait this long while reloading textures before we raise the curtain
...
void display(BOOL rebuild, F32 zoom_factor, int subfield, BOOL for_snapshot)
{
	...
	//	const F32 TELEPORT_ARRIVAL_DELAY = 0.f; // Time to preload the world before raising the curtain after we've actually already arrived.
		...
		case LLAgent::TELEPORT_START:
			...
			gViewerWindow->setShowProgress(TRUE); // kirsten
	...
	// display_stats(); // More spam in the debug console FPS can be found elsewhere! kirsten
