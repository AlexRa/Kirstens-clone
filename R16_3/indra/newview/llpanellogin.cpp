...
const S32 BLACK_BORDER_HEIGHT = 260;
...
void LLLoginHandler::parse(const LLSD& queryMap)
{
	...
	if (queryMap["grid"].asString() == "OpenLife")
	{
		grid_choice = GRID_INFO_OPENLIFE;
	}
	else if (queryMap["grid"].asString() == "OLBeta")
	{
		grid_choice = GRID_INFO_OLBETA;
	}
	else if (queryMap["grid"].asString() == "Local")
	{
		grid_choice = GRID_INFO_LOCAL;
	}
	...
	mHtmlAvailable( TRUE )
{
	...
	// Need to handle login openlife:///app/ URLs
	web_browser->setOpenAppURL3DXs( true );
