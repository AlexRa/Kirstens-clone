...
#ifdef LL_WINDOWS
	static char sWindowClass[] = "Open Life";
#endif
...
bool send_url_to_other_instance(const std::string& url)
{
	...
	if (other_window != NULL)
	{
		...
		COPYDATASTRUCT cds;
		const S32 URL3DX_MESSAGE_TYPE = 0;
		cds.dwData = URL3DX_MESSAGE_TYPE;
...
bool LLAppViewer::init()
{
	...
	gDirUtilp->initAppDirs("OpenLife"); //Kirsten
...
bool LLAppViewer::initLogging()
{
	...
	// Set the log file to OpenLife.log KL
...
{	
	...
	gSavedSettings.setString("VersionChannelName", DX_CHANNEL);
	...
	if(!initParseCommandLine(clp))
	{
		...
		std::ostringstream msg;
		msg << "Open Life found an error parsing the command line. \n" 
			<< "Please see: http://openlifegrid.com/MyToolbox/SupportBox/tabid/185/Default.aspx \n"
	...
	// Handle url3DX use. NOTE: Don't let SL-55321 reappear.

    // *FIX: This init code should be made more robust to prevent 
    // the issue SL-55321 from returning. One thought is to allow 
    // only select options to be set from command line when a url3DX 
    ...
    {
        std::string url3DX = clp.getOption("url")[0];
        if (LLURLDispatcher::isURL3DXCommand(url3DX))
        {
	        LLStartUp::sURL3DXCommand = url3DX;
        }
        else
        {
	        LLURLSimString::setString(url3DX);
        }
    }
    else if(clp.hasOption("url3DX"))
    {
        std::string url3DX = clp.getOption("url3DX")[0];
        if(LLURLDispatcher::isURL3DX(url3DX))
        {
            if (LLURLDispatcher::isURL3DXCommand(url3DX))
            {
	            LLStartUp::sURL3DXCommand = url3DX;
	...
	//	// Legacy string.  This flag really meant we didn't want to expose references to "Open Life".
	...
	std::string url3DX;
	if (!LLStartUp::sURL3DXCommand.empty())
	{
		url3DX = LLStartUp::sURL3DXCommand;
	}
	else if (LLURLSimString::parse())
	{
		url3DX = LLURLSimString::getURL();
	}
	if (!url3DX.empty())
	{
		if (send_url_to_other_instance(url3DX))
		{
...
bool LLAppViewer::initWindow()
{
	...
	gViewerWindow = new LLViewerWindow(gWindowTitle, "Open Life", //Kirsten
...
bool LLAppViewer::initCache()
{
	...
	if (LL_VERSION_MAJOR >= 1 && LL_VERSION_MINOR >= 21)
	{
		...
		{
			// NOTE: (Nyx) as of 1.21, cache for mac is moving to /library/caches/OpenLife from
			// /library/application support/OpenLife/cache This should clear/delete the old dir.
			std::string cache_dir = gDirUtilp->getOSUserAppDir();
			std::string new_cache_dir = gDirUtilp->getOSCacheDir();
			cache_dir = cache_dir + "/cache";
			new_cache_dir = new_cache_dir + "/OpenLife";  // Watch this on darwin Build KL! may cause issues with cmake
