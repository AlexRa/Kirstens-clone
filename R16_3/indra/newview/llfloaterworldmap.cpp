...
BOOL LLFloaterWorldMap::postBuild()
{
	...
	childSetAction("copy_url3DX", onCopyURL3DX, this);
...
//	childSetEnabled("Clear", (BOOL)tracking_status);
	...
	childSetEnabled("copy_url3DX", (mURL3DX.size() > 0) ); //Kirsten url3DX
...
void LLFloaterWorldMap::updateLocation()
{
	...
	if (pos_global.isExactlyZero())
	{
		...
		if ( status == LLTracker::TRACKING_NOTHING && mSetToUserPosition )
		{
			...
			if ( gotSimName )
			{
				...
				// Set the current URL3DX
				mURL3DX = LLURLDispatcher::buildURL3DX(agent_sim_name, agent_x, agent_y, agent_z); //kirsten url3DX
			}
		}
		...
		(status != mTrackedStatus || pos_global != mTrackedLocation || sim_name != mTrackedSimName))
	{
		...
		// simNameFromPosGlobal can fail, so don't give the user an invalid URL3DX
		if ( gotSimName )
		{
			mURL3DX = LLURLDispatcher::buildURL3DX(sim_name, llround(region_x), llround(region_y), llround((F32)pos_global.mdV[VZ]));
		}
		else
		{	// Empty URL3DX will disable the "Copy URL3DX to clipboard" button
			mURL3DX = "";
		}
	}
}
...
void LLFloaterWorldMap::onClearBtn(void* data)
{
	...
	LLWorldMap::getInstance()->mIsTrackingUnknownLocation = FALSE;
	self->mURL3DX = "";				// Clear the URL3DX since it's invalid
...
// static
void LLFloaterWorldMap::onCopyURL3DX(void* data)
{
	LLFloaterWorldMap* self = (LLFloaterWorldMap*)data;
	gViewerWindow->mWindow->copyTextToClipboard(utf8str_to_wstring(self->mURL3DX)); //kirsten url3DX
	
	LLStringUtil::format_map_t args;
	args["[URL3DX]"] = self->mURL3DX;

	LLAlertDialog::showXml("CopyURL3DX", args); 
}
