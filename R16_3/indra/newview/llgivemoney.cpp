...
void LLFloaterPay::processPayPriceReply(LLMessageSystem* msg, void **userdata)
{
	...
	if (self)
	{
		...
		for (i=0;i<num_blocks;++i)
		{
			...
			if (pay_button > 0)
			{
				std::string button_str = "C-"; //Kirsten
		...
		std::string balance_str = "C-"; //Kirsten
...
void LLFloaterPay::payViaObject(money_callback callback, const LLUUID& object_id)
{
	...
	LLFloaterPay *floater = new LLFloaterPay("Give C-", callback, object_id, TRUE);
	if (!floater) return;
...
void LLFloaterPay::payDirectly(money_callback callback, 
							   ...
							   BOOL is_group)
{
	LLFloaterPay *floater = new LLFloaterPay("Give C-", callback, target_id, FALSE);
	if (!floater) return;
