...
void inventory_offer_handler(LLOfferInfo* info, BOOL from_task)
{
	...
	// Strip any URL3DX from the message display. (DEV-2754)
	std::string msg = info->mDesc;
	int indx = msg.find(" ( http://url3DX.com/openlife/");
	if(indx >= 0)
	{
...
void process_improved_im(LLMessageSystem *msg, void **user_data)
{
	...
	if (prefix == "/me " || prefix == "/me'")
	{
        ...
        // Kirsten handle DXCMD style messages.
	std::string dxcmd = message.substr(0, 4);
	if (prefix == "### " || prefix == "###'")
	{
		separator_string = "";
		message_offset = 3;
	...
	case IM_NOTHING_SPECIAL: 
		...
		{
			// Messages from "Open Life" ID don't go to IM history
...
void process_chat_from_simulator(LLMessageSystem *msg, void **user_data) //KL
{
	LLChat		chat;
	std::string		mesg;
	std::string		from_name;
...
void process_agent_movement_complete(LLMessageSystem* msg, void**)
{
	...
	if( is_teleport )
	{
		...
		if (avatarp)
		{
			// Chat the "back" URL3DX. (DEV-4907)
			LLChat chat("Teleport completed from " + gAgent.getTeleportSourceURL3DX());
...
void process_money_balance_reply( LLMessageSystem* msg, void** )
{
	...
	LL_INFOS("Messaging") << "Cr, credit, committed: " << balance << " " << credit << " "
			<< committed << LL_ENDL; //kirsten
...
void process_script_teleport_request(LLMessageSystem* msg, void**)
{
	...
	// LLURLDispatcher::dispatch(LLURLDispatcher::buildURL3DX(sim_name, (S32)pos.mV[VX], (S32)pos.mV[VY], (S32)pos.mV[VZ]), FALSE);
...
void process_covenant_reply(LLMessageSystem* msg, void**)
{
	...
	else
	{
		...
		{
			covenant_text = "There is no Covenant provided for this Estate. The land on this estate is being sold by the Estate owner.  Please contact the Estate Owner for sales details.";
		}
