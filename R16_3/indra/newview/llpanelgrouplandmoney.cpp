 ...
 * @brief Panel for group land and credits.
 *
...
void LLPanelGroupLandMoney::update(LLGroupChange gc)
{
	...
	if ( tabp )
	{
		...
		//now pull the event handler associated with that C- tab
		if ( panelp )
		{
...
BOOL LLPanelGroupLandMoney::postBuild()
{
	...
	*/
	...
	//pull out the widgets for the C- details tab
	...
	//pull out the widgets for the C- sales tab
...
void LLPanelGroupLandMoney::processPlacesReply(LLMessageSystem* msg, void**)
{
	...
	if(!selfp)
	{
		llinfos << "Group Panel Land C- " << group_id << " no longer in existence."
				<< llendl;
		return;
	}
															...
															void** data)
{
	...
	{
		llwarns << "Got group C- history reply for another agent!" << llendl;
		return;
	}
															...
															void** data)
{
	...
	if (!self)
	{
		llwarns << "GroupAccountSummary recieved for non-existent group C- planning tab." << llendl;
		return;
	}
