...
BOOL LLWindowWin32::switchContext(BOOL fullscreen, const LLCoordScreen &size, BOOL disable_vsync, const LLCoordScreen * const posp)
{
	...
	if (pfd.cAlphaBits < 8)
	{
		close();
		OSMessageBox(
			"OpenLife is unable to run because it can't get an 8 bit alpha\n"
	...
	if (!gGLManager.initGL())
	{
		close();
		OSMessageBox(
					 "OpenLife is unable to run because your video card drivers\n"
					 ...
					 "If you continue to receive this message, contact OpenLife with a support ticket.",  //KL fix for Sai :)
					 "Error",
					 OSMB_OK);
		return FALSE;
	}
