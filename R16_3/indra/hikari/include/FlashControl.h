...
public:
   ...
   * @return False if movie file not found or if flash version < 9, true otherwise
	*
	* @note	The specified movie should reside in the "assetsDirectory" that
	*		was declared when the HikariManager was instantiated.
	*/
	bool load(const Ogre::String& movieFilename);

   /**
	* Loads a movie (a .swf file) into this FlashControl and begins playing.
	*
	* @param	movieFilename	Full absolute path of the movie to load.
   * @return False if movie file not found or if flash version < 9, true otherwise
	*/
   bool loadDirect(const Ogre::String& movieFilename);
