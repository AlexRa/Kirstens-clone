...
typedef enum ELLPath
{
    ...
    LL_PATH_USER_FACE_EXPRESSIONS = 17,
	LL_PATH_FACE_EXPRESSIONS = 18,          // Switch around not to break path executable!
	LL_PATH_LAST
} ELLPath;
...
// pure virtual functions
	...
	const std::string &getAppName() const;			// install directory under progams/ ie "OpenLife"
	...
	// reX: new function for rex-style username
	virtual void setPerAccountChatLogsDir(const std::string &user);		// Set the per user chat log directory.
	// reX: new function for rex-style username
	virtual void setLindenUserDir(const std::string &user);		// Set the linden user dir to this user's dir
...
protected:
	std::string mAppName;               // install directory under progams/ ie "OpenLife"   //Kirsten
