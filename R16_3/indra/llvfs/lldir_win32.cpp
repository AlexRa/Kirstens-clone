...
void LLDir_Win32::initAppDirs(const std::string &app_name)
{
	...
	// reX
	res = LLFile::mkdir(getExpandedFilename(LL_PATH_USER_FACE_EXPRESSIONS,""));
	if (res == -1)
	{
		if (errno != EEXIST)
		{
			llwarns << "Couldn't create LL_PATH_USER_FACE_EXPRESSIONS dir " << getExpandedFilename(LL_PATH_USER_FACE_EXPRESSIONS,"") << llendl;
		}
	}
