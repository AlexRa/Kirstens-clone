...
const std::string  LLDir::getCacheDir(bool get_default) const
{
	if (mCacheDir.empty() || get_default)
	{
		...
		{
			res = getOSCacheDir() + mDirDelimiter + "OpenLife"; //Kirsten
		}
		return res;
	}
	else
...
std::string LLDir::getExpandedFilename(ELLPath location, const std::string& subdir1, const std::string& subdir2, const std::string& in_filename) const
{
	...
	case LL_PATH_EXECUTABLE:
		...
		break;
			...
			// reX: new directories
    case LL_PATH_FACE_EXPRESSIONS:
		prefix = getAppRODataDir();
		prefix += mDirDelimiter;
		prefix += "expressions";
		break;

    case LL_PATH_USER_FACE_EXPRESSIONS:
		prefix = getOSUserAppDir();
		prefix += mDirDelimiter;
		prefix += "expressions";
		break;		
...
// reX: new function
void LLDir::setLindenUserDir(const std::string &user)
{
	if (!user.empty())
	{
		// some platforms have case-sensitive filesystems, so be
		...
		LLStringUtil::toLower(userlower);   //rex merge changed string types KL
		for (unsigned i = 0; i < userlower.size(); i++)
		{
			if (userlower[i] == ' ')
				userlower[i] = '_';
		}
		...
		mLindenUserDir = getOSUserAppDir();
		mLindenUserDir += mDirDelimiter;
		mLindenUserDir += userlower.c_str();
	}
	else
	{
...
// reX: new function
void LLDir::setPerAccountChatLogsDir(const std::string &user)
{
	if (!user.empty())
	{
		// some platforms have case-sensitive filesystems, so be
		...
		mPerAccountChatLogsDir = getChatLogsDir();
		mPerAccountChatLogsDir += mDirDelimiter;
		mPerAccountChatLogsDir += userlower.c_str();
	}
	else
	{
		llwarns << "Invalid name for LLDir::setPerAccountChatLogsDir" << llendl;
