...
{
    // reX: we are paranoid, because Ogre uses VBO's, it's better to unbind always
    // probably fixes Nvidia threading optimization bug 
	if (sVBOActive || gGLManager.mHasVertexBufferObject)
	{
		glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
		sVBOActive = FALSE;
	}
	if (sIBOActive || gGLManager.mHasVertexBufferObject)
	{
